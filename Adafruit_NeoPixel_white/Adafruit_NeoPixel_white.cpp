...
Adafruit_NeoPixel_white::Adafruit_NeoPixel_white(uint16_t n, uint8_t p, uint8_t t) : numLEDs(n), numBytes(n), pin(p), type(t), pixels(NULL)
#ifdef __AVR__
...
void Adafruit_NeoPixel_white::begin(void) {
  pinMode(pin, OUTPUT);
  digitalWrite(pin, LOW);
}

void Adafruit_NeoPixel_white::show(void) {
...
void Adafruit_NeoPixel_white::setPixelColor(
 uint16_t n, uint8_t w) {
  if(n < numLEDs) {
...
uint8_t Adafruit_NeoPixel_white::getPixelColor(uint16_t n) {
...
uint16_t Adafruit_NeoPixel_white::numPixels(void) {
  return numLEDs;
}
...
void Adafruit_NeoPixel_white::setBrightness(uint8_t b) {
